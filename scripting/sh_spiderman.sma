...
spiderman_health 300
spiderman_armor 300
spiderman_gravity 0.75
...
public plugin_init()
{
	...
	register_cvar("spiderman_teamglow", "0")
	
	register_cvar("spiderman_health", "300")
	register_cvar("spiderman_armor", "300")

	// FIRE THE EVENT TO CREATE THIS SUPERHERO!
	shCreateHero(gHeroName, "Telaraña!", "Obtén Telaraña, Balanceate entre edificios con tu telaraña. - Pone en say /bind para aprender a bindear.", true, "spiderman_level" )
	...
	shSetMaxHealth(gHeroName, "spiderman_health")
	shSetMaxArmor(gHeroName, "spiderman_armor")
...
public plugin_precache()
{
	...
	precache_model("models/player/spidermanendgamel/spidermanendgamel.mdl")
...
public newRound(id)
{
	if ( gHasSpiderPower[id] && is_user_alive(id) && shModActive() ) 
	{
		spider_tasks(id)
	} 
...
public spiderman_death()
{
	...
	if ( !gHasSpiderPower[id] ) return
	cs_set_user_model(id, "spidermanendgamel")
...
{
	set_task(1.0, "spider_morph", id)

	if( get_cvar_num("spiderman_teamglow") ){
		set_task(1.0, "spider_glow", id+gTaskID, "", 0, "b" )
	}
}
//----------------------------------------------------------------------------------------------
public spider_morph(id)
{
	...
	show_hudmessage(id, "Ahora sos Spider-Man End Game!.")

	gmorphed[id] = true
}
//----------------------------------------------------------------------------------------------
public spider_unmorph(id)
{
	if ( gmorphed[id] ) {
		// Message
		set_hudmessage(50, 205, 50, -1.0, 0.40, 2, 0.02, 4.0, 0.01, 0.1, 7)
		show_hudmessage(id, "Ya no sos más Spider Man.")
...
public spider_glow(id)
{
	...
	if ( gHasSpiderPower[id] && is_user_alive(id)) {
		if ( get_user_team(id) == 1 ) {
			shGlow(id, 255, 0, 0)
		}
		else {
			shGlow(id, 70, 130, 210)
		}
	}
}
//----------------------------------------------------------------------------------------------
...
{
	if ( damagebits & DMG_FALL && gHasSpiderPower[this] ) return HAM_SUPERCEDE
