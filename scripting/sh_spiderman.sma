...
spiderman_health 300		// set hp and ap with the hero
...
new bool:gHasSpiderPowers[SH_MAXSLOTS+1]
...
new g_spriteWeb, gMoveacc, gRealspeed, gHookstyle, gHooksky, gTeamColored, gMaxhooks

// models for the hero
new const model_name[] 		= "spiderman_eg" 
new const model_precache[] 	= "models/player/spiderman_eg/spiderman_eg.mdl"

new const g_sprite_precache[] 	= "sprites/zbeam4.spr"
new const g_soundWeb[] 		= "bullchicken/bc_bite2.wav"

// the tick dont fall damage
new bool:isCzero
...
{
	// Plugin Info
	register_plugin("SUPERHERO Spider-Man","1.3","{HOJ} Batman/JTP10181")

	// DO NOT EDIT THIS FILE TO CHANGE CVARS, USE THE SHCONFIG.CFG
	new pcvarLevel	= register_cvar("spiderman_level", "9" )
	gMoveacc	= register_cvar("spiderman_moveacc", "680" )
	gRealspeed	= register_cvar("spiderman_reelspeed", "1000" )
	gHookstyle	= register_cvar("spiderman_hookstyle", "3" )
	gHooksky	= register_cvar("blackwidow_hooksky", "0" )
	gTeamColored	= register_cvar("spiderman_teamcolored", "1" )
	gMaxhooks	= register_cvar("spiderman_maxhooks", "-1" )
	new pcvarHPnAP	= register_cvar("spiderman_health", "300")

	// FIRE THE EVENT TO CREATE THIS SUPERHERO!
	gHeroID = sh_create_hero(gHeroName, pcvarLevel);
	sh_set_hero_info(gHeroID, "Telaraña!", "Obtén Telaraña, Balanceate entre edificios con tu telaraña. - Pone en say /bind para aprender a bindear.");
...
{
	g_spriteWeb = precache_model(g_sprite_precache) 
	precache_sound(g_soundWeb) 
...
{ 
	...
	switch(mode) {
		case SH_HERO_ADD: {
			gHasSpiderPowers[id] = true
			Spider_morph_unmorph(id) 
		}
		case SH_HERO_DROP: {
			gHasSpiderPowers[id] = false
			Spider_morph_unmorph(id)
			Spider_hookOff(id)
		}
	}
...
{ 
	...
	if ( !is_user_alive(id) || !gHasSpiderPowers[id] ) return 
	
	switch(key) {
		case SH_KEYDOWN: {
			if ( PassAimTest(id) ) Spider_hookOn(id)
		}
		case SH_KEYUP: {
			Spider_hookOff(id)
		}
...
{
	if ( gHasSpiderPowers[id] ) {
		Spider_tasks(id)
		g_hooksLeft[id] = get_pcvar_num(gMaxhooks)
		Spider_hookOff(id)
	} 
...
{
	if ( gHasSpiderPowers[victim] )
		Spider_hookOff(victim)
}
//------------------------------------------------------------------------------------------------
//					Task Morph and Unmorph					//
//------------------------------------------------------------------------------------------------
public Spider_tasks(id) 
	set_task(1.0, "Spider_morph_unmorph", id)

public Spider_morph_unmorph(id) 
{
	if ( gHasSpiderPowers[id] ) cs_set_user_model(id, model_name) 
	else cs_reset_user_model(id)
}
//----------------------------------------------------------------------------------------------
// 				For check the sky aim the hook
//----------------------------------------------------------------------------------------------
PassAimTest(id) 
{
	new origin[3]
	...
	if (AimAt == CONTENTS_SKY && !get_pcvar_num(gHooksky)) {
...
//----------------------------------------------------------------------------------------------
public Spider_physics(id, bool:autoReel)
{
	...
	buttonpress = entity_get_int(id, EV_INT_button)
	...
	if (buttonadjust[0] || buttonadjust[1]) {
		...
		velocity[0] += move_direction[0] * get_pcvar_float(gMoveacc) * HOOK_DELTA_T / get_distance(null,move_direction)
		velocity[1] += move_direction[1] * get_pcvar_float(gMoveacc) * HOOK_DELTA_T / get_distance(null,move_direction)
	}
	if (buttonadjust[2] < 0 || (buttonadjust[2] && g_hookLength[id] >= 60)) {
		g_hookLength[id] -= floatround(buttonadjust[2] * get_pcvar_float(gRealspeed) * HOOK_DELTA_T)
	}
...
public Spider_cheapReel(id)
{
	...
	if ( distance > 60 ) {
		velocity[0] = (g_hookLocation[id][0] - user_origin[0]) * ( 1.0 * get_pcvar_num(gRealspeed) / distance )
		velocity[1] = (g_hookLocation[id][1] - user_origin[1]) * ( 1.0 * get_pcvar_num(gRealspeed) / distance )
		velocity[2] = (g_hookLocation[id][2] - user_origin[2]) * ( 1.0 * get_pcvar_num(gRealspeed) / distance )
	}
	else {
		velocity[0] = 0.0
		velocity[1] = 0.0
...
public Spider_hookOn(id)
{
	...
	// this is fo rcontrol on max hooks
	if ( g_hooksLeft[id]== 0 ) {
	...
	beamentpoint(id)
	emit_sound(id, CHAN_STATIC, g_soundWeb, 0.7, ATTN_NORM, 0, PITCH_NORM)
	
	parm[0] = id
	parm[1] = get_pcvar_num(gHookstyle)
	set_task(HOOK_DELTA_T, "Spider_checkWeb", id, parm, 2, "b")
}

public Spider_checkWeb(parm[])
{
	new id = parm[0]
	new style = parm[1]

	if (style==1) Spider_physics(id, false)
	if (style==2) Spider_physics(id, true)
	if (style>2 || style < 0 ) Spider_cheapReel( id )
}

public Spider_hookOff(id)
{
	killbeam(id)
	remove_task(id)
}
...
//----------------------------------------------------------------------------------------------
//		This is from The Tick for dont fall damage
//----------------------------------------------------------------------------------------------
public ham_TakeDamage(this, inflictor, attacker, Float:damage, damagebits)
{
	if ( damagebits & DMG_FALL && gHasSpiderPowers[this] ) return HAM_SUPERCEDE
