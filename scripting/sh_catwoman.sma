...
new gPcvarSeconds, gPcvarChange, gPcvarMode, gPcvarPctPerLvl, gPcvarMaxRounds, gPcvarMinRounds
//----------------------------------------------------------------------------------------------
public plugin_init()
{
	// Plugin Info
	...
	new pcvarLevel 	= register_cvar("catwoman_level", "1")
	gPcvarSeconds	= register_cvar("catwoman_seconds", "0.5")
	gPcvarChange	= register_cvar("catwoman_change", "10.0")
	gPcvarMode	= register_cvar("catwoman_mode", "2")
	gPcvarPctPerLvl	= register_cvar("catwoman_pctperlev", "0.02")
	gPcvarMaxRounds	= register_cvar("catwoman_maxrounds", "20")
	gPcvarMinRounds	= register_cvar("catwoman_minrounds", "5")

	// FIRE THE EVENT TO CREATE THIS SUPERHERO!
	gHeroID = sh_create_hero(gHeroName, pcvarLevel)
	sh_set_hero_info(gHeroID, "Spawnea en la base de los enemigos.", "Tenes una chance de meterte a la base del enemigo cada principio de ronda.")
...
{
	if ( gHasCatWomanPowers[id] && is_user_alive(id) ) {
		if ( get_pcvar_float(gPcvarChange) > 0.0 ) cs_reset_user_model(id)

		if ( get_pcvar_num(gPcvarMode) == 1 ) {
			new randNum = random_num(0, 100)
			new level = floatround(gPlayerLevels[id] * get_pcvar_float(gPcvarPctPerLvl) * 100)
			if ( level > randNum )
			...
			new sneakLevel, max = get_pcvar_num(gPcvarMaxRounds), min = get_pcvar_num(gPcvarMinRounds)
		...
		if ( gCatWomanmns[id] && gCatWomanSneak[id] ) {
			...
			set_task(get_pcvar_float(gPcvarSeconds), "catwoman_sneak", id)
		}
	}
}
//----------------------------------------------------------------------------------------------
...
{
	if ( !is_user_alive(id) || !gHasCatWomanPowers[id] ) return

	new entSpawn
	new Float:spawnAngle[3]
	new Float:spawnOrigin[3]
	new Float:changetime = get_pcvar_float(gPcvarChange)
	new CsTeams:idTeam = cs_get_user_team(id)
	...
	if ( !entSpawn ) {
		...
		sh_chat_message(id, gHeroID, "No hay forma de meterse en la base enemiga.")
		return
	}
	...
	sh_chat_message(id, gHeroID, "Lograste meterte en la base enemiga.")
...
public catwoman_changeback(id)
{
	...
	sh_chat_message(id, gHeroID, "Perdiste tu traje de infiltrado.")
}
//----------------------------------------------------------------------------------------------
