...
// GLOBAL VARIABLES
new gHeroID
new gHeroName[] = "Goku's Kaio-Ken Technic"
...
//pcvar holders
new goku_speed, goku_hp, goku_ap, goku_time, goku_cooldown, g_spritePowerUp2, gMsgSync
// para desbugear la velocidad cuando se da el boost
new g_lastWeapon[SH_MAXSLOTS+1], g_prevWeapon[SH_MAXSLOTS+1] 
...
public plugin_init() 
{
	...
	goku_ap 	= register_cvar("gokukt_armor", "1000")
...
public plugin_precache()
{
	...
	precache_sound("shmod/kaiokengokukt.wav")
}
//------------------------------------------------------------------------------------------------
//				Hero INIT and KEY						//
//------------------------------------------------------------------------------------------------
...
{ 
	...
	if ( !is_user_alive(id) || !HasGokuKT[id] ) return PLUGIN_HANDLED;
	...
	if ( key == SH_KEYDOWN ) {
		...
		if ( GokuKTTimer[id] > 0 ) return PLUGIN_HANDLED;
		
		GokuKTTimer[id] = get_pcvar_num(goku_time) + 1
		gokukt_kaiokenmode(id)
		
		emit_sound(id, CHAN_WEAPON, "shmod/kaiokengokukt.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
...
public sendGokuKTCooldown(id)
{
	...
	if (gPlayerInCooldown[id])
		cooldown = floatround( get_pcvar_num(goku_cooldown) + get_pcvar_num(goku_time) + 1  - get_gametime() + GokuKTUsedTime[id] + 0.4 )
	else
		cooldown = -1
	return cooldown
}
...
{
	if ( !shModActive() || !HasGokuKT[id] || !is_user_alive(id) ) return

	kaioken[id] = false
	
	new set_health = get_pcvar_num(goku_hp) - get_user_health(id)
	new set_armor = get_pcvar_num(goku_ap) - get_user_armor(id)
	if ( set_health < 1 ) set_health = 750;
	if ( set_armor < 1 ) set_armor = 750;
...
public gokukt_kaiokenmode(id)
{
	...
	if ( HasGokuKT[id] && is_user_alive(id) ) {
		...
		set_hudmessage(255, 0, 0, -1.0, 0.3, 0, 0.0, 1.0, 0.0, 0.0, 4)
		// show_hudmessage(id, "10 Times x Kaioken! Haaaa!")
