			...
			for ( i = 0; i < count; i++ )
			{
				...
				if ( is_user_alive(id) )
				{
						...
						// Sets Y location
							...
							//agregados
							set_hudmessage(0, 128, 0, 0.02, 0.97, 0, 0.0, 0.3, 0.0, 0.0)
							ShowSyncHudMsg(id, MonitorHudSync, "[SH] Health: %d  |  Armor: %d^nGravity: %d%%  |  Speed: %d", UserHealth[id], UserArmor[id], floatround(gravity*100.0), floatround(vector_length(velocity)))
						#else
							set_hudmessage(0, 128, 0, 0.02, 0.73, 0, 0.0, 0.3, 0.0, 0.0) 
							ShowSyncHudMsg(id, MonitorHudSync, "[SH]  %s", temp)	//agregado
						#endif
