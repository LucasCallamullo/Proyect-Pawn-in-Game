...
new g_spriteWeb, gRealspeed, gHookstyle, gHooksky, gTeamColored, gMaxhooks // gMoveacc

// models for the hero
new const model_name[] 		= "blackwidow"
new const model_precache[] 	= "models/player/blackwidow/blackwidow.mdl"

new const g_sprite_precache[] 	= "sprites/zbeam4.spr"
new const g_soundWeb[] 		= "weapons/xbow_hit2.wav"
...
public plugin_init()
{
	...
	// gMoveacc	= register_cvar("blackwidow_moveacc", "680" )
	gRealspeed	= register_cvar("blackwidow_reelspeed", "1000" )
	gHookstyle	= register_cvar("blackwidow_hookstyle", "3" )
	gHooksky	= register_cvar("blackwidow_hooksky", "0" )
	gTeamColored	= register_cvar("blackwidow_teamcolored", "1" )
	gMaxhooks	= register_cvar("blackwidow_maxhooks", "-1" )
...
{
	if ( gHasBlackwidowPowers[victim] ) {
		blackwidow_hookOff(victim)
		blackwidow_morph_unmorph(victim)
	}
}
//------------------------------------------------------------------------------------------------
...
public blackwidow_tasks(id) 
	set_task(1.0, "blackwidow_morph_unmorph", id)

public blackwidow_morph_unmorph(id) 
{
	if ( gHasBlackwidowPowers[id] ) cs_set_user_model(id, model_name) 
...
public blackwidow_physics(id, bool:autoReel)
{
	...
	if (buttonadjust[0] || buttonadjust[1]) {
		...
		velocity[0] += move_direction[0] * get_pcvar_float(gRealspeed) * HOOK_DELTA_T / get_distance(null,move_direction)
		velocity[1] += move_direction[1] * get_pcvar_float(gRealspeed) * HOOK_DELTA_T / get_distance(null,move_direction)
	}
