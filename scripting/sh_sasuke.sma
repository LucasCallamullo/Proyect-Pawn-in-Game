...
{ 
	...
	if ( key == SH_KEYDOWN ) {
		...
		emit_sound(id, CHAN_WEAPON, "shmod/katun.wav", 0.6, ATTN_NORM, 0, PITCH_NORM)
...
public on_fire(args[])
{
	...
	if ( LevelVictim <= 16 ) {
		...
		if(health - damage  <= 0) {
			...
			sh_chat_message(id, -1, "[%s] Te está quemando con su Bola de Fuego! (Katon!).", attackerName)
		}
	}
}
//----------------------------------------------------------------------------------------------
...
{
	if (!is_user_alive(victim) && gHasPowers[victim] )  {
		gIsBurning[victim] = 0;
