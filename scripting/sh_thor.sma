...
public plugin_init()
{
	...
	gPcvarDamage	= register_cvar("thor_pctofdmg", "90")
	gPcvarCooldown	= register_cvar("thor_cooldown", "25")
...
public sh_client_death(id) 
{
	...
	if ( g_hasThor[id] ) gPcvarRealCD[id] = sh_get_cooldown(id)
}
//----------------------------------------------------------------------------------------------
//			DAMAGE EVENT
//----------------------------------------------------------------------------------------------
public thor_damage(id)
{
	...
	if ( sh_user_has_hero(attacker, sh_get_hero_id(gOthers_Heros[0])) ) return
	...
	if ( is_user_alive(attacker) && !get_user_godmode(attacker) && id != attacker ) {
		...
		new extraDamage = floatround(damage * get_pcvar_num(gPcvarDamage) * 0.01 )
