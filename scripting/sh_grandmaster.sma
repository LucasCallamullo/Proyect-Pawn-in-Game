...
public plugin_init()
{
	...
	sh_set_hero_info(gHeroID, "Revive a los aliados.", "Utiliza la Fuerza Cósmica de la Vida para revivir a tus compañeros de equipo.")
}
...
public sh_client_death(victim)
{
	...
	for ( new i = 0; i < playerCount; i++ ) {
		...
		if ( player != victim && gHasGrandmaster[player] && !gPlayerInCooldown[player] && idTeam == cs_get_user_team(player) ) {
			...
			uchiha revenge = 1.2
			torneo = 1.5 */
			set_task(1.1, "gmaster_respawn", _, parm, 2)
		}
	}
}
//----------------------------------------------------------------------------------------------
public gmaster_respawn(parm[])
{
	...
	sh_chat_message(0, gHeroID, "%s Uso la Fuerza Cósmica para revivir a su Compañero Muerto: %s!", gmasterName, deadName)

	//Respawns the player best available method
	//tambien hace que se ejecute antes de tiempo que los demas
	ExecuteHamB(Ham_CS_RoundRespawn, dead)
	
	//Double spawn prevents the no HUD glitch (hopefully) PRUEBA PARA BOTS
	//user_spawn(dead)
	//user_spawn(dead)
