...
public plugin_init(){
	...
	g_show_adv2 = register_cvar("amx_fre_adv","4");
	g_show_adv3 = register_cvar("amx_fre_adv","1");
	g_recive_weapons = register_cvar("amx_fre_rw","0");
}
...
public Round_Timer()
{
	...
	g_adv_time = (g_roundtime - 21); //se supon que de aca controlas el timer cada cuanto se muestra u know
	if(get_pcvar_num(toggle_plugin))
	{
		set_task(float(g_roundtime), "force_end",TASK_ID);
		if(get_pcvar_num(g_show_adv2))
			{
			set_task(float(g_adv_time), "show_adv2",TASK_ID);
			}
	}	
	
	g_adv_time = (g_roundtime - 31); //se supon que de aca controlas el timer cada cuanto se muestra u know
	if(get_pcvar_num(toggle_plugin))
	{
		set_task(float(g_roundtime), "force_end",TASK_ID);
		if(get_pcvar_num(g_show_adv3))
			{
			set_task(float(g_adv_time), "show_adv3",TASK_ID);
			}
	}	
...
public show_adv()
{
	//set_hudmessage(50, 205, 50, -1.0, 0.40, 2, 0.02, 4.0, 0.01, 0.1, 7) // hud de los models plarey
	//set_hudmessage(252, 1, 1, -1.0, -1.0, 1, 6.0, 3.0,_,_,-1); // hud en la mira
	set_hudmessage(0, 255, 26, -1.0, 0.28, 2, 0.02, 2.3, _, _, -1) // hud del clin esatwood
	show_hudmessage(0, "Tenes 10 segundos para ganar la ronda, sino todos morirán.");
...
{
	//set_hudmessage(50, 205, 50, -1.0, 0.40, 2, 0.02, 4.0, 0.01, 0.1, 7) // hud de los models plarey
	//set_hudmessage(255, 247, 0, -1.0, -1.0, 1, 6.0, 4.0,_,_,-1); // hud en la mira
	set_hudmessage(0, 255, 26, -1.0, 0.28, 2, 0.02, 2.3, _, _, -1) // hud del clin esatwood
	show_hudmessage(0, "Tenes 20 segundos para ganar la ronda, sino todos morirán.");
...
{
	//set_hudmessage(50, 205, 50, -1.0, 0.40, 2, 0.02, 4.0, 0.01, 0.1, 7) // hud de los models plarey
	//set_hudmessage(0, 255, 26, -1.0, -1.0, 1, 6.0, 4.0,_,_,-1); // hud en la mira
	set_hudmessage(0, 255, 26, -1.0, 0.28, 2, 0.02, 2.3, _, _, -1) // hud del clin esatwood
	show_hudmessage(0, "Tenes 30 segundos para ganar la ronda, sino todos morirán.");
