...
broly_power_1 70		    //Max Damage for ssjlevel 1 power (Default 70)
broly_power_2 100		    //Max Damage for ssjlevel 2 power (Default 100)
broly_power_3 175		    //Max Damage for ssjlevel 3 power (Default 175)
broly_power_4 300		    //Max Damage for ssjlevel 4 power (Default 300)

broly_radius_1 100		//Max Radius of Damage for ssjlevel 1th power (Default 100)
broly_radius_2 300		//Max Radius of Damage for ssjlevel 2th power (Default 300)
broly_radius_3 700		//Max Radius of Damage for ssjlevel 3th power (Default 700)
broly_radius_4 1500		//Max Radius of Damage for ssjlevel 4th power (Default 1500)
...
{
	// Plugin Info
	register_plugin("SUPERHERO broly", "0.2", "|RIC|_ALBANIAN / 0 / vittu / RD")

	// DO NOT EDIT THIS FILE TO CHANGE CVARS, USE THE SHCONFIG.CFG
	new pcvarLevel	= register_cvar("broly_level", "10")
	register_cvar("broly_aps", "50")
	register_cvar("broly_ap_mult", "250")
	register_cvar("broly_hp_mult", "100")
	register_cvar("broly_hp_max", "2000")
	register_cvar("broly_speed_base", "300")
	register_cvar("broly_speed_add", "25")
	register_cvar("broly_power_1", "50")
	register_cvar("broly_power_2", "80")
	register_cvar("broly_power_3", "150")
	register_cvar("broly_power_4", "300")
	register_cvar("broly_radius_1", "100")
	register_cvar("broly_radius_2", "300")
	register_cvar("broly_radius_3", "700")
	register_cvar("broly_radius_4", "1500")
	register_cvar("broly_decals", "1")
	register_cvar("broly_maxaps", "1300")
...
{
	precache_model("models/player/broly/broly.mdl")
	// precache_sound("shmod/vegeto_galitgun.wav")
	precache_sound("shmod/broly_galitgun.wav")
	precache_sound("shmod/broly_finalflashb.wav")
	precache_sound("shmod/broly_bigbang.wav")
	precache_sound("shmod/broly_deathball.wav")
	precache_sound("shmod/broly_powerup1.wav")
	precache_sound("shmod/broly_powerup2.wav")
	precache_sound("shmod/broly_powerup3.wav")
	precache_sound("shmod/broly_powerup4.wav")
	precache_sound("player/pl_pain2.wav")
	precache_model("sprites/shmod/gallitguna2bro.spr")
	precache_model("sprites/shmod/finalflashchargebro.spr")
	precache_model("sprites/shmod/bigbangbro.spr")
	precache_model("sprites/shmod/deathball2bro.spr")
	g_spriteTrailY = precache_model("sprites/shmod/gallitguntrail2bro.spr")
	g_spriteTrailB = precache_model("sprites/shmod/finalflashtrailbro.spr")
	g_spriteTrailR = precache_model("sprites/shmod/bigbangtrailbro.spr")
	g_spriteExplosionG = precache_model("sprites/shmod/gallitguna2bro.spr")
	g_spriteExplosionB = precache_model("sprites/shmod/finalflashbbro.spr")
	g_spriteExplosionR = precache_model("sprites/shmod/bigbangexp2bro.spr")
	g_spriteExplosionO = precache_model("sprites/shmod/deathball2bro.spr")
	g_spritePowerUp = precache_model("sprites/shmod/broly_powerup4.spr")
...
public loadCVARS()
{
	...
	g_maxarmorPts = get_cvar_num("broly_maxaps")
	g_armorPts = get_cvar_num("broly_aps")
	g_ssjLevel[0] = get_cvar_num("broly_ap_mult")
	g_ssjLevel[1] = g_ssjLevel[0] * 2
	g_ssjLevel[2] = g_ssjLevel[0] * 3
	g_ssjLevel[3] = g_ssjLevel[0] * 4
	g_ssjSpeed[0] = get_cvar_float("broly_speed_base")
	g_ssjSpeed[1] = g_ssjSpeed[0] + get_cvar_float("broly_speed_add")
	g_ssjSpeed[2] = g_ssjSpeed[1] + get_cvar_float("broly_speed_add")
	g_ssjSpeed[3] = g_ssjSpeed[2] + get_cvar_float("broly_speed_add")
	...
	switch(key) {
		case SH_KEYDOWN: {
			...
			if ( userArmor < g_ssjLevel[0] ) {
				...
				sh_chat_message(id, -1, "[%s] No tenes suficiente Ki, Recarga Armor/Ki." , g_heroName)
				return
			}
			...
			if( g_powerID[id] ){
				playSoundDenySelect(id)
				sh_chat_message(id, -1, "[%s] No podes usar más de un poder a la vez." , g_heroName)
				return
			}
			...
			if ( userArmor >= g_ssjLevel[0] && userArmor < g_ssjLevel[1] ) {
				sh_chat_message(id, -1, "[%s] Galitgun!" , g_heroName)
				...
				g_maxDamage[id] = get_cvar_num("broly_power_1")
				g_maxRadius[id] = get_cvar_num("broly_radius_1")
				g_powerNum[id] = 1
			}
			else if ( userArmor >= g_ssjLevel[1] && userArmor < g_ssjLevel[2] ) {
				sh_chat_message(id, -1, "[%s] Final Flash!!" , g_heroName)
				// Wish this sound was shorter
				emit_sound(id, CHAN_STATIC, "shmod/broly_finalflashb.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
				set_user_armor(id, userArmor-g_ssjLevel[1])
				g_maxDamage[id] = get_cvar_num("broly_power_2")
				g_maxRadius[id] = get_cvar_num("broly_radius_2")
				g_powerNum[id] = 2
			}
			else if ( userArmor >= g_ssjLevel[2] && userArmor < g_ssjLevel[3] ) {
				sh_chat_message(id, -1, "[%s] Big Bang!!" , g_heroName)
				// Wish this sound was shorter
				emit_sound(id, CHAN_STATIC, "shmod/broly_bigbang.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
				set_user_armor(id, userArmor-g_ssjLevel[2])
				g_maxDamage[id] = get_cvar_num("broly_power_3")
				g_maxRadius[id] = get_cvar_num("broly_radius_3")
				g_powerNum[id] = 3
			}
			else if ( userArmor >= g_ssjLevel[3] ) {
				// Remove Users glowing since he was ssjlevel 4
				set_user_rendering(id)
				sh_chat_message(id, -1, "[%s] Death-Ball!!!" , g_heroName)
				
				emit_sound(id, CHAN_STATIC, "shmod/broly_deathball.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
				set_user_armor(id, userArmor-g_ssjLevel[3])
				g_maxDamage[id] = get_cvar_num("broly_power_4")
				g_maxRadius[id] = get_cvar_num("broly_radius_4")
				g_powerNum[id] = 4
			}
...
public sh_client_spawn(id)
{
	...
	if ( shModActive() && g_hasbroly[id]  && is_user_alive(id) ) {	
		broly_tasks(id)
		// Set armor in x seconds to avoid breaking max ap settings in other heroes
		set_task(0.5, "broly_setarmor", id)
...
{
	if ( g_hasbroly[victim] ) 
		cs_set_user_model(victim, "broly")
}

public broly_tasks(id)
	set_task(1.0, "broly_morph", id)

public broly_morph(id)
{
	if ( gmorphed[id] || !is_user_alive(id) ) return

	cs_set_user_model(id, "broly")
	gmorphed[id] = true
}

public broly_unmorph(id)
{
	if ( gmorphed[id] ) {
...
public create_power(id)
{
	...
	switch(g_powerNum[id]){
		case 1:{
			entModel = "sprites/shmod/gallitguna2bro.spr"
			entScale = 0.60
			entSpeed = 1800.0
		...
		case 2:{
			entModel = "sprites/shmod/finalflashchargebro.spr"
			entScale = 0.60
			entSpeed = 1600.0
		...
		case 3:{
			entModel = "sprites/shmod/bigbangbro.spr"
			entScale = 1.0
			entSpeed = 1050.0
		...
		case 4:{
			entModel = "sprites/shmod/deathball2bro.spr"
			entScale = 1.70
			entSpeed = 850.0
			VecMins[0] = -4.0
			VecMins[1] = -4.0
	...
	Entvars_Set_String(newEnt, EV_SZ_classname, "vexd_broly_power")
...
public vexd_pfntouch(pToucher, pTouched) {
	...
	if(equal(szClassName, "vexd_broly_power")) {
		...
		if ( get_cvar_num("broly_decals") == 1 ) {
...
public broly_loop()
{
	...
	for (new i = 0; i < pnum; i++) {
		id = players[i]
		if ( g_hasbroly[id] && is_user_alive(id) ) {
			...
			if ( userArmor < g_ssjLevel[0] ) {
				...
				if ( g_isSaiyanLevel[id] > 0 ) {
					shResetSpeed(id)
					set_hudmessage(204, 0, 204, -1.0, 0.25, 0, 0.25, 3.0, 0.0, 0.0, 3)
					show_hudmessage(id, "[Broly] - En este momento estás juntando Ki.")
				...
				if ( g_isSaiyanLevel[id] < 1 ) {
					...
					set_hudmessage(0, 255, 0, -1.0, 0.25, 0, 0.25, 3.0, 0.0, 0.0, 3)
					show_hudmessage(id, "[Broly] - Te Trasformaste en Broly SSJ.")
					emit_sound(id, CHAN_STATIC, "shmod/broly_powerup1.wav", 0.8, ATTN_NORM, 0, PITCH_NORM)
				...
				if ( g_isSaiyanLevel[id] < 2 ) {
					...
					set_hudmessage(255, 165, 0, -1.0, 0.25, 0, 0.25, 3.0, 0.0, 0.0, 3)
					show_hudmessage(id, "[Broly] - Te Trasformaste en Broly SSJ 2.")
					emit_sound(id, CHAN_STATIC, "shmod/broly_powerup2.wav", 0.8, ATTN_NORM, 0, PITCH_NORM)
				...
				if ( g_isSaiyanLevel[id] < 3 ) {
					...
					set_hudmessage(0, 255, 255, -1.0, 0.25, 0, 0.25, 3.0, 0.0, 0.0, 3)
					show_hudmessage(id, "[Broly] - Te Trasformaste en el Legendario Broly SSJ 3.")
					emit_sound(id, CHAN_STATIC, "shmod/broly_powerup2.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
				...
				if ( g_isSaiyanLevel[id] < 4 ) {
					...
					set_hudmessage(255, 165, 0, -1.0, 0.25, 0, 0.25, 3.0, 0.0, 0.0, 3)
					show_hudmessage(id, "[Broly] - Te Trasformaste en el Legendario Broly SSJ 4.")
					emit_sound(id, CHAN_STATIC, "shmod/broly_powerup4.wav", 0.8, ATTN_NORM, 0, PITCH_NORM)         
        
					g_isSaiyanLevel[id] = 4
					shake_n_stun(id)
					ssj_boost(id)
					sh_set_godmode(id, 2.5)
				}
			}
		}
	}
...
public ssj_boost(id)
{
	if ( !shModActive() || !g_hasbroly[id] || !is_user_alive(id) || !sh_is_inround()  ) return
	...
	if ( userHealth < get_cvar_num("broly_hp_max") ) {
		new addHP = get_cvar_num("broly_hp_mult") * g_isSaiyanLevel[id]
		if ( userHealth + addHP > get_cvar_num("broly_hp_max") ) {
			set_user_health(id, get_cvar_num("broly_hp_max") )
			}
		else 	{
...
public shake_n_stun(id)
{
	...
	for (new i = 0; i < pnum; i++) {
		...
		if ( distance <= get_cvar_num("vegetto_radius_4") ) {
			...
			shStun(vic, 0)
			set_user_maxspeed(vic, 740.0)

			set_task(3.0, "reset_instun", vic)

			if ( vic == id ) continue
			// Let them know why they get shaken and stunned, except the person that leveled
			new brolyName[32]
			get_user_name(id, brolyName, 31)
			set_hudmessage(196, 0, 0, -1.0, 0.20, 0, 0.25, 3.0, 0.0, 0.0, 3)
			show_hudmessage(vic, "[Broly] - %s se transformo en el Legendary Super Saiyajin 4", brolyName)
		}
	}
}
//----------------------------------------------------------------------------------------------
public reset_instun(id)
{
	...
	if ( !shModActive() || !g_hasbroly[id] || !is_user_alive(id) ) return
...
public client_disconnected(id)
{
	...
	if( g_hasbroly[id] && g_powerID[id] > 0 ) {
		remove_power(id, g_powerID[id])
	}
}
//----------------------------------------------------------------------------------------------
