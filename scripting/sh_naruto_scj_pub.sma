...
new bool:gNarutoSelected[SH_MAXSLOTS+1]
#if SEND_COOLDOWN
	new Float:NarutoscjUsedTime[SH_MAXSLOTS+1]
#endif
//-----------------------------------------------------------------------------------------
public plugin_init()
{
	...
	pCvarrdamage		= 	register_cvar("naruto_rdamage", "120")
	...
	pCvarHitChance 		= 	register_cvar("naruto_hitchance", "40")
	pCvarCloneHealth	=	register_cvar("naruto_clonehealth", "200")
	register_cvar("naruto_adminflag", "p")
...
public sh_hero_init(id, heroID, mode)
{
	...
	switch(mode) {
		case SH_HERO_ADD: {
			...
			gNarutoSelected[id] = gHasNaruto[id]
...
public sh_hero_key(id, heroID, key)
{
	...
	if ( key == SH_KEYDOWN ) 
	{
		...
		{
			sh_chat_message(id, gHeroID, "Ya has invocado a un clon de sombra, estara aqui en breve.")
			sh_sound_deny(id)
			return
		}
		...
		{
			sh_chat_message(id, gHeroID, "Necesitas descansar antes de usar tu Rasengan de nuevo.")
			sh_sound_deny(id)
			return
		}
		else if (botEnt[id])
		...
		{
			sh_chat_message(id, gHeroID, "Tenes que estar vivo para realizar el jutsu clon de sombra.")
			sh_sound_deny(id)
			return
		}
		else if(bot_userid != 0)
		{
			...
			sh_chat_message(id, gHeroID, "Has invocado un Shadow Clone of %s para luchar por vos ahora!", name)
			
			new name2[32]
			get_user_name(id, name2, charsmax(name2))

			sh_chat_message(bot_userid, gHeroID, "%s Han invocado tu Shadow Clone para luchar por ellos!", name2)
...
public sendNarutoscjCooldown(id)
{
	...
	if (gPlayerInCooldown[id])
		cooldown = floatround( get_pcvar_num(pCvarCooldown) - get_gametime() + NarutoscjUsedTime[id] + 0.4 )
	else
		cooldown = -1
	return cooldown
}
...
public sh_client_spawn(id)
{
	...
	{	
		...
		if( pev_valid(botEnt[id]) )
		{
			...
			sh_chat_message(id, gHeroID, "La ronda termino, tu clon de sombra ahora desaparecera.")
		}
	}
...
public shadowclone_over(id) 
{
	if (botEnt[id] && bot_userid != 0)
	{
		...
		if (pev_valid(botEnt[id])) 
		{
			...
			sh_chat_message(id, gHeroID, "Tu Shadow Clone fue fiel por %d segundos. El ahora desaparecera.", activetime)
		}
	}
}
//-----------------------------------------------------------------------------------------
public shadowclone(id)
{
	if (!is_user_alive(id)) 
	{
		sh_chat_message(id, gHeroID, "Tenes que estar vivo para realizar el jutsu clones de sombra.")
		sh_sound_deny(id)
	...
	sh_chat_message(id, gHeroID, "Tu Shadow Clone ha llegado.")
...
public give_weapon(ent,id)
{
	...
	entity_set_model(entWeapon[id], "models/shmod/darthmaul_p_knife.mdl")
			...
			if(ent==botEnt[i])
			{
				if (pev_valid(botEnt[i]) && pev_valid(ent))
				{
					...
					if(!pev_valid(botEnt[i]) && gHasNaruto[i] && ent==botEnt[i])
					{
						...
						sh_chat_message(i, gHeroID, "Tu clon de sombra deserto! Ahora desaparecera.")
					...
					if(pev_valid(ent) && ent==botEnt[i])
					{
						...
						if (rasenganover[i] == true)
						{
							...
							entity_set_model(entWeapon[i], "models/shmod/darthmaul_p_knife.mdl")
...
public target_found(ent,owner,target)
{
	...
	if (doingrasengan[owner] == true)
	{
		...
		if(get_distance_f(entOrigin,targetOrigin)<60.0)
		{
			...
			{
				entity_set_model(entWeapon[owner], "models/shmod/darthmaul_p_knife.mdl")
...
//--------------------- AGREGADO PARA QUE SEA SOLO D ADMIN EL PODER --------------------------------------------------------------------
public Naruto_admincheck(id) 
{
   	new accessLevel[10] 
	
	get_cvar_string("naruto_adminflag", accessLevel, 9)
   	
	if ( gNarutoSelected[id] &&  !(get_user_flags(id)&read_flags(accessLevel)) ) {
