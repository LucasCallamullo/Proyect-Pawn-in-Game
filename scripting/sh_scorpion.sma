...
public plugin_init()
{
	...
	new pcvarLevel	= register_cvar("scorpion_level", "9")
	register_cvar("scorpion_maxhooks", "50")
	register_cvar("scorpion_reelspeed", "1100")
	register_cvar("scorpion_dragmates", "0")
	register_cvar("scorpion_knifefight", "0")
	register_cvar("scorpion_mode", "1")
	register_cvar("scorpion_uppercutdmg", "100")
	register_cvar("scorpion_speardmg", "100")
	register_cvar("scorpion_stuntime", "2.5")
	
	// dont use this not found
	gPcvarCooldown = register_cvar("scorpion_cooldown", "4.0")

	// FIRE THE EVENT TO CREATE THIS SUPERHERO!
	gHeroID = sh_create_hero(g_heroName, pcvarLevel);
	sh_set_hero_info(gHeroID,  "Get Over Here!", "Apunta tu Harpoon y Atrae a tus enemigos a vos.");
...
{ 
	...
	switch(mode) {
		case SH_HERO_ADD: {
			g_hasScorpionPower[id] = true
			gPlayerInCooldown[id] = false
			if ( g_hooked[id] ) scorpion_hookOff(id)
		}
		case SH_HERO_DROP: {
			g_hasScorpionPower[id] = false;
			if ( g_hooked[id] ) scorpion_hookOff(id)
		}
	}
}
...
{ 
	...
	if ( !is_user_alive(id) || !g_hasScorpionPower[id] ) return;
	...
	switch(key) {
		case SH_KEYDOWN: {
			...
			// if(get_cvar_float("scorpion_cooldown") > 0.0) ultimateTimer(id, get_cvar_float("scorpion_cooldown"))
...
#if SEND_COOLDOWN
public sendScorpionCooldown(id)
{
...
public sh_client_spawn(id)
{
	...
	if (g_hasScorpionPower[id]) {
		
		// This is from scorpion functions
		...
		if ( gPcvarRealCD[id] > 0.0 ) {
			sh_set_cooldown(id, gPcvarRealCD[id])
		}
		else {
...
public scorpion_hookOn(id)
{
	...
	if ( g_hooksLeft[id] == 0 ) {
		playSoundDenySelect(id)
		sh_chat_message(id, -1, "[%s] No te quedan más Scorpion Spears, espera a la siguiente ronda.", g_heroName)
		return
	}

	if ( g_hooksLeft[id] > 0 ) g_hooksLeft[id]--

	if ( g_hooksLeft[id] >= 0 && g_hooksLeft[id] < 10 ) {
		sh_chat_message(id, -1, "[%s] Te quedan %d Scorpion Spears, quedan: %d .", g_heroName, g_hooksLeft[id], (g_hooksLeft[id] == 1) ? "" : "s")
	}
	...
	if ( hooktarget > 0 && hooktarget < 33 ) {
		...
		if ( get_user_team(hooktarget) != get_user_team(id) ) {
			...
			if ( mode >= 2 ) {
				...
				set_user_maxspeed(hooktarget, 360.0)
			}
...
public scorpion_reelin(parm[])
{
	...
	set_task(1.0, "scorpion_hookOff", id)
...
public scorpion_fight_on(id, enemy)
{
	...
	sh_chat_message(id, -1, "[%s] Mortal Kombat! Duelo de cuchillos hasta la muerte con %s.", g_heroName, enemy_name)
	sh_chat_message(id, -1, "[%s] FIGHT!", g_heroName)
	...
	for (new i = 1; i <= SH_MAXSLOTS; i++) {
		...
		if ( fight_found ) {
			...
			if ( !other_fight ) {
				...
				sh_chat_message(i, -1, "%s WINS", i_name)
				// client_print(i, print_center, "%s WINS", i_name)
				if ( is_user_connected(id) ) {
					sh_chat_message(id, -1, "%s WINS", i_name)
					// client_print(id, print_center, "%s WINS", i_name)
				}
