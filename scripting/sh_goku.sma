...
goku_maxaps 1300		//en teoria para setear el max de ap a juntar
...
public plugin_init()
{
	...
	register_cvar("goku_maxaps", "1300")

	// FIRE THE EVENT TO CREATE THIS SUPERHERO!
	shCreateHero(g_heroName, "Llega a transformarte en SSJ 4!", "Regenerá KI/Armor para Transformarte en distintas Fases de SSJ. Obtén un Poder Especial y HP/Speed por cada Fase de SSJ.", true, "goku_level")
	...
	register_event("DeathMsg", "goku_death", "a") 
}
//----------------------------------------------------------------------------------------------
public plugin_precache()
{
	precache_model("models/player/goku4/goku4.mdl")
	...
	g_spritePowerUp2 = precache_model("sprites/shmod/esf_powerup4.spr")
...
//----------------------------------------------------------------------------------------------
public goku_tasks(id)
{
	set_task(1.0, "goku_morph", id)
}
//----------------------------------------------------------------------------------------------
public goku_morph(id)
{
	if ( gmorphed[id] || !is_user_alive(id) ) return

	cs_set_user_model(id, "goku4")

	// Message
	set_hudmessage(50, 205, 50, -1.0, 0.40, 2, 0.02, 4.0, 0.01, 0.1, 7)
	show_hudmessage(id, "Ahora sos el Legendario Goku SSJ4.")

	gmorphed[id] = true
}
//----------------------------------------------------------------------------------------------
public goku_unmorph(id)
{
	if ( gmorphed[id] ) {
		// Message
		set_hudmessage(50, 205, 50, -1.0, 0.40, 2, 0.02, 4.0, 0.01, 0.1, 7)
		show_hudmessage(id, "Ya no sos més Goku.")
...
public loadCVARS()
{
	...
	g_maxarmorPts = get_cvar_num("goku_maxaps")
...
public goku_kd()
{
	...
	if ( userArmor < g_ssjLevel[0] ) {
		...
		sh_chat_message(id, -1, "[%s] No tenes suficiente KI/Armor." , g_heroName)
		return
	}
	...
	if( g_powerID[id] ){
		playSoundDenySelect(id)
		sh_chat_message(id, -1, "[%s] No podes usar més de un poder a la vez." , g_heroName)
		return
	}
	...
	if ( userArmor >= g_ssjLevel[0] && userArmor < g_ssjLevel[1] ) {
		sh_chat_message(id, -1, "[%s] Ki Blast!" , g_heroName)
		...
		sh_chat_message(id, -1, "[%s] Kame Hame Ha!!" , g_heroName)
		// Wish this sound was shorter
		...
		sh_chat_message(id, -1, "[%s] Kame Hame Ha x10 KaioKen!!!" , g_heroName)
		// Wish this sound was shorter
		...
		sh_chat_message(id, -1, "[%s] Genki Dama!!!" , g_heroName)
		// Wish this sound was shorter
...
public create_power(id)
{
	...
	if( newEnt == 0 ) {
		client_print(id, print_chat, "[SH](Goku) Fallo la creacion de tu poder.")
		return
	}
		...
		if ( g_hasGoku[id] && is_user_alive(id) ) {
			...
			if ( userArmor < g_ssjLevel[0] ) {
				...
				if ( g_isSaiyanLevel[id] < 1 ) {
					...
					show_hudmessage(id, "Goku - Te transformaste en Super Saiyajin.")
				...
				if ( g_isSaiyanLevel[id] < 2 ) {
					...
					show_hudmessage(id, "Goku - Te Transformaste en Super Saiyajin 2.")
				...
				if ( g_isSaiyanLevel[id] < 3 ) {
					...
					show_hudmessage(id, "Goku - Te Transformaste en Super Saiyajin 3!")
				...
				if ( g_isSaiyanLevel[id] < 4 ) {
					...
					parm[1] = 11
					powerup_effect(parm)
					set_task(0.1, "powerup_effect2", 0, parm, 2, "a", 79)

					set_hudmessage(196, 0, 0, -1.0, 0.25, 0, 0.25, 3.0, 0.0, 0.0, 84)
					show_hudmessage(id, "Goku - Te Transformaste en Super Saiyajin 4!")
...
public ssj_boost(id)
{
	...
	//if ( get_user_maxspeed(id) < g_ssjSpeed[speedNum] && !g_inStun[id] ) {      // el signo original es este  "<"
...
public shake_n_stun(id)
{
	...
	for (new i = 0; i < pnum; i++) {
		...
		if ( distance <= get_cvar_num("goku_radius4") ) {
			...
			show_hudmessage(vic, "Goku - %s Te transformaste en Super Saiyajin 4", gokuName)
		}
	}
}
//----------------------------------------------------------------------------------------------
