...
#if SEND_COOLDOWN
	new Float:VaderUsedTime[SH_MAXSLOTS+1]
#endif
//------------------------------------------------------------------------------------------------
...
{ 
	...
	switch(key) {
		...
		case SH_KEYUP: {
			...
			if ( !g_grabbedID[id] ) sh_chat_message(id, gHeroID, "Ha parado la busqueda.")
...
public sendVaderCooldown(id)
{
	...
	if (gPlayerInCooldown[id])
		cooldown = floatround( get_pcvar_num(pcvarCooldown) - get_gametime() + VaderUsedTime[id] + 0.4 )
	else
		cooldown = -1
	return cooldown
}
...
{
	if ( g_hasVader[victim] && !is_user_alive(victim) ) {
