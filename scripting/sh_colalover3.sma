...
new maxPlayers, gmsgScreenFade, gmsgStatusIcon, /* gmsgBlinkAcct, gmsgAmmoPickup, gmsgTextMsg, gmsgWeapPickup */
...
new const gCocaWeapon1[] = "models/shmod/cokegrenade_v.mdl"
new const gCocaWeapon2[] = "models/shmod/cokegrenade_p.mdl"  

#if SEND_COOLDOWN
	new Float:ColaLoverUsedTime[SH_MAXSLOTS+1]
#endif
//------------------------------------------------------------------------------------------------
...
public plugin_init() 
{
	...
	new pcvarArmor 		=  register_cvar("coca_armor", "900")
	...
	// pcv_limit 	= register_cvar("fn_limit","0");	//pelotudes
	// pcv_buyzone 	= register_cvar("fn_buyzone","1");
	// pcv_price 	= register_cvar("fn_price","300");	// precio si el override = 0
...
public plugin_precache()
{
	...
	precache_model("models/shmod/cokegrenade_v.mdl")
	precache_model("models/shmod/cokegrenade_p.mdl")
	precache_model("models/shmod/cokegrenade_w.mdl")
}
//-----------------------------------------------------------------------------
...
{
	if(!get_pcvar_num(pcv_enabled) || get_pcvar_num(pcv_override)) return PLUGIN_CONTINUE;

	if ( !shModActive() || !is_user_alive(id) || !gHasColaLoverPower[id] ) return PLUGIN_HANDLED

	/ if(get_pcvar_num(pcv_buyzone) && !cs_get_user_buyzone(id) && gHasColaLoverPower[id]) {
	...
	if(!(get_pcvar_num(pcv_teams) & _:cs_get_user_team(id)) && gHasColaLoverPower[id]) {
	...
	/ if(money < price && gHasColaLoverPower[id]) {
...
public sh_client_spawn(id)
{
	...
	if(get_pcvar_num(pcv_icon) == ICON_HASNADE ) { //&& gHasColaLoverPower[id]
		new status = player_has_frostnade(id);
		show_icon(id, status);
	}
	
	if ( shModActive() && gHasColaLoverPower[id] ) {
...
public sendColaLoverCooldown(id)
{
	...
	if (gPlayerInCooldown[id])
		cooldown = floatround( get_pcvar_num(CvarGrenadeReset) - get_gametime() + ColaLoverUsedTime[id] + 0.4 - 0.3)
	else
		cooldown = -1
	return cooldown
}
#endif
//-----------------------------------------------------------------------------------------------
public coca_weapon(id)
{
	if ( shModActive() && is_user_alive(id) && gHasColaLoverPower[id] ) {
...
{
	if ( !is_user_alive(id) || !gHasColaLoverPower[id] ) return
    
	new weaponID = read_data(2)
	if (weaponID !=CSW_FLASHBANG) return
	switch_model(id)
}
//----------------------------------------------------------------------------------------------
switch_model(id)
{
	if (get_user_weapon(id) == CSW_FLASHBANG) {
		set_pev(id, pev_viewmodel2, gCocaWeapon1)
		set_pev(id, pev_weaponmodel2, gCocaWeapon2)
	}
}
//Fin? 
//----------------------------------------------------------------------------------------------
	...
	if ( iAmmoType == AMMOX_FLASHBANG ) {
		if ( iAmmoCount == 0 && !BlockGiveNade[id] ) {
			...
			if ( !gPlayerInCooldown[id] ) {
				...
				while ( (iGrenade = find_ent_by_class(iGrenade, "grenade")) > 0 ) {
					...
					if ( id == entity_get_edict(iGrenade, EV_ENT_owner) && equal(model, "models/w_flashbang.mdl") ) {
						entity_set_model(iGrenade, "models/shmod/cokegrenade_w.mdl")
					}
				}
			}
			...
			if ( seconds > 0.0 ) {
				// Proof From Hud			
				#if SEND_COOLDOWN
					ColaLoverUsedTime[id] = get_gametime()
				#endif	
...
{
	if(pev_valid(orig)) { //&& gHasColaLoverPower[orig]
