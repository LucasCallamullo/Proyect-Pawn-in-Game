...
public plugin_init()
{
	...
	register_clcmd("say /dropmenu", "DropMenu")		// Para llamar al menu de fakas
	register_clcmd("say dropmenu", "DropMenu")		// Para llamar al menu de fakas
	register_clcmd("say dropmenu", "DropMenu")		// Para llamar al menu de fakas
	// Touch Forward
...
menuSuperPowers(id, menuOffset)
{
	...
	//formatex(message, 68, "\yMenú de Super Poderes:%-16s\r(Seleccionaste %d/%d)^n^n", " ", playerpowercount, total)
	...
	if ( gPlayerMenuChoices[id][0] > 8 ) {
		...
		add(message, charsmax(message), "\w^n9. Más Héroes")
		keys |= MENU_KEY_9
	}
	else {
		add(message, charsmax(message), "^n")
...
public selectedSuperPower(id, key)
{
	...
	if ((gPlayerBinds[id][0] >= MaxBinds && gSuperHeros[heroIndex][requiresKeys])) {
		chatMessage(id, _, "No podes elegir más de %d Héroes que requieran Binds.", MaxBinds)
		menuSuperPowers(id, gPlayerMenuOffset[id])
		return PLUGIN_HANDLED
	}
	else if ( gMaxPowersLeft[id][heroLevel] <= 0 ) {
		chatMessage(id, _, "No podes elegir más Héroes de ese nivel.")
		menuSuperPowers(id, gPlayerMenuOffset[id])
		return PLUGIN_HANDLED
	}
	...
	set_hudmessage(100, 100, 0, -1.0, 0.2, 0, 1.0, 6.0, 0.1, 0.2, 4)	// defaultu
...
public cl_clearpowers(id)
{
	...
	if ( !get_pcvar_num(sh_alivedrop) && is_user_alive(id) ) {
		...
		chatMessage(id, _, "No podes dropear Héroes mientras estés vivo.")
		return PLUGIN_HANDLED
	}
	...
	if ( gGiveMercyXP && !gReadXPNextRound[id] && !gBlockMercyXp[id] ) {
		...
		if ( mercyxpmode != 0 && gPlayerStartXP[id] >= gPlayerXP[id] && get_playersnum() > get_pcvar_num(sh_minplrsbhxp) ) {
			...
			if ( XPtoGive != 0 ) {
				...
				chatMessage(id, _, "Se te otorgaron %d MercyXP, por no haber ganado XP la ronda anterior.", XPtoGive)
			}
		}
...
public cl_say(id)
{
	...
	if ( equali(said[pos], "superherohelp") || equali(said[pos], "help") ) {
	...
	else if ( equali(said[pos], "myheroes") || equali(said[pos], "heroes") || equali(said[pos], "poderes") ) {
		showHeroes(id)
		return PLUGIN_HANDLED
	}
		...
		clearAllPowers(id, true)
		chatMessage(id, _, "Todos tus Héroes han sido dropeados exitosamente.")
		return PLUGIN_HANDLED
	}
	else if ( equali(said[pos], "showmenu") || equali(said[pos], "menu") ) {
		menuSuperPowers(id, 0)
		return PLUGIN_HANDLED
	}
		...
		chatMessage(id, _, "Por ayuda con el SuperHero Mod, say: /ayuda o /help.")
		return PLUGIN_CONTINUE
	}
...
dropPower(id, const said[])
{
	...
	if ( spaceIdx > 0 && strlen(said) > spaceIdx+2 ) {
		...
		chatMessage(id, _, "Proporcione al menos 2 letras del nombre del Héroe que desea dropear.")
		return
	}
	...
	if ( found ) {
		...
		chatMessage(id, _, "No se pudo encontrar un Héroe con ese nombre para dropear: %s", heroName)
	}
}
//----------------------------------------------------------------------------------------------
showHelp(id)
...
showHelpHud()
{
	...
	set_hudmessage(230, 100, 10, 0.80, 0.28, 0, 1.0, 1.0, 0.9, 0.9, 4)  // defaultu
...
public host_Killed()
{
	...
	chatMessage(id, _, "Perdiste %d XP por matar a un rehén.", XPtoTake)
	displayPowers(id, false)
}
//----------------------------------------------------------------------------------------------
public host_Rescued()
{
	...
	localAddXP(id, XPtoGive)
	chatMessage(id, _, "Obtuviste %d XP por rescatar a un rehén.", XPtoGive)
	displayPowers(id, false)
}
//----------------------------------------------------------------------------------------------
public host_AllRescued()
{
	...
	for (new i = 0; i < numplayers; i++) {
		...
		localAddXP(ct, XPtoGive)
		chatMessage(ct, _, "Tu equipo obtuvo %d XP por rescatar a todos los rehenes.", XPtoGive)
		displayPowers(ct, false)
	}
}
//----------------------------------------------------------------------------------------------
...
public bomb_planted(planter)
{
	...
	chatMessage(planter, _, "Obtuviste %d XP por plantar la bomba.", XPtoGive)
...
public bomb_defused(defuser)
{
	...
	chatMessage(defuser, _, "Obtuviste %d XP por defusear la bomba.", XPtoGive)
...
public bomb_explode(planter, defuser)
{
	...
	for (new i = 0; i < numplayers; i++) {
		...
		chatMessage(terrorist, _, "Tu equipo obtuvo %d XP por una explosión de bomba exitosa.", XPtoGive)
...
public DropMenu(id)
{
    ...
    formatex(string, 200, "Menú Para Dropear Poderes", id )
    ...
    formatex(string, 128, "Volver", id )
    menu_setprop(menu, MPROP_BACKNAME, string);
    formatex(string, 128, "Más Poderes", id )
    menu_setprop(menu, MPROP_NEXTNAME, string);
    formatex(string, 128, "Cancelar", id )
