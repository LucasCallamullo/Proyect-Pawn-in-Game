...
shock_armor 400			//Armor Starts with (def=250)
...
{
	// Plugin Info
	register_plugin("SUPERHERO Frozono","1.0","Neogaiden(z)/Strife")

	// DO NOT EDIT THIS FILE TO CHANGE CVARS, USE THE SHCONFIG.CFG
	register_cvar("shock_level", "0" )
	register_cvar("shock_timer", "0.1" )
	register_cvar("shock_maxspeed", "980" )
	register_cvar("shock_armorfuel", "400" )
	register_cvar("shock_fuelcost","1")
	register_cvar("shock_armor","400")

	// FIRE THE EVENT TO CREATE THIS SUPERHERO!
	shCreateHero(gHeroName, "Movete rápido con hielo.", "Dónde está mi super traje mujer? - Pone en say /bind para aprender a bindear.", true, "shock_level" )
...
public shock_loop(id)
{
	...
	if ( gUseFuel != 0 && gFuelCost > userArmor && g_jetPackRunning[id] == 1 ) {
		...
		sh_chat_message(id, -1, "[%s] Te deshidrataste! - Toma un poco de agua y espera a recuperar energia/armor." , gHeroName)
		removeMark(id)
		return PLUGIN_HANDLED
	}
	...
	if (g_jetPackRunning[id] == 1) {
		...
		if ( sqrt(distance2[0]*distance2[0]+distance2[1]*distance2[1] == 0 || distance2[0]*distance2[0]+distance2[1]*distance2[1] == 0) ) return PLUGIN_CONTINUE
...
public make_trail(id,origin2[3])
{
	...
	set_user_rendering(id,kRenderFxGlowShell,50,50,255, kRenderNormal, 16)
