...
public plugin_init()
{
	...
	sh_set_hero_info(gHeroID, "Invisibilidad.", "Podes hacerte invisible, solamente no tenes que salir del piso o atacar druante un tiempo, ideal para campear.")
...
public remInvisibility(id)
{
	...
	if ( gIsInvisible[id] == 100 ) {
		sh_set_rendering(id)
		//client_print(id, print_center, "[SH]%s: Ya no estas en modo invisible.", gHeroName)
		set_hudmessage(50, 205, 50, -1.0, 0.40, 0, 0.02, 1.0, 0.01, 0.1, 3)
		show_hudmessage(id, "[SH][%s]: Ya no estás en modo invisible.", gHeroName)
	}
	gIsInvisible[id] = 0
}
//----------------------------------------------------------------------------------------------
		...
		if ( setVisible ) {
			...
			if ( sysTime - delay >= gStillTime[id] ) {
				if ( gIsInvisible[id] != 100 ) {
					...
					show_hudmessage(id, "[SH][%s]: 100%s cargado.", gHeroName, "%")
					}
				...
				if ( gIsInvisible[id] >= 90 ) {
					set_hudmessage(50, 205, 50, -1.0, 0.40, 2, 0.02, 4.0, 0.01, 0.1, 3)
					show_hudmessage(id, "[SH][%s]: %d%s cargado.", gHeroName, rPercent, "%")
				}
