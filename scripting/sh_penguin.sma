...
new gSpriteTrail, gPcvarCooldown, gPcvarMult, gPcvarFuse, gPcvarNadeSpeed

// This is for cooldowns
new Float:gPcvarRealCD[SH_MAXSLOTS+1] 

new const gPenguin_v[] = "models/shmod/penguin_v_hegrenade.mdl"
new const gPenguin_w[] = "models/shmod/penguin_w_hegrenade.mdl"
...
public plugin_init()
{
	...
	new pcvarLevel	= register_cvar("penguin_level", "5")
	
	gPcvarCooldown	= register_cvar("penguin_cooldown", "25.0")
	gPcvarMult	= register_cvar("penguin_grenademult", "2.5")
	gPcvarFuse	= register_cvar("penguin_fuse", "6.0")
	gPcvarNadeSpeed	= register_cvar("penguin_nadespeed", "950")

	// FIRE THE EVENTS TO CREATE THIS SUPERHERO!
	gHeroID = sh_create_hero(gHeroName, pcvarLevel)
	sh_set_hero_info(gHeroID, "HE-Penguins Rastreadoras.", "Obtén una HE grenade con Extra Daño, que persigue a tus enemigos, y que se recarga cada cierto tiempo.")
	
	// REGISTER EVENTS THIS HERO WILL RESPOND TO! (AND SERVER COMMANDS)
	// weapons models chaneg
...
public plugin_precache()
{
	...
	precache_model(gPenguin_v)
	precache_model(gPenguin_w)
}
//------------------------------------------------------------------------------------------------
//					INIT y SPAWN						//
//------------------------------------------------------------------------------------------------
...
{
	if ( is_user_alive(id) && gHasPenguinPower[id] ) {
		gPlayerInCooldown[id] = false
...
{
	if ( gHasPenguinPower[victim] && !is_user_alive(victim) )
		gPlayerInCooldown[victim] = false	 
}

#if SEND_COOLDOWN
public sendPenguinCooldown(id)
{
...
public HeGrenade_Deploy(iEnt)
{
	new id = get_pdata_cbase(iEnt, 41, 4)	// 41 y 4 son constantes van siempre
	if ( !is_user_alive(id) || !gHasPenguinPower[id] ) return HAM_IGNORED; 
	
	set_pev(id, pev_viewmodel2, gPenguin_v) 
	...
	if ( iAmmoType == AMMOX_HEGRENADE && gHasPenguinPower[id] ) {
		...
		if ( iAmmoCount == 0 ) {
			...
			if ( !gPlayerInCooldown[id] ) { 
				...
				while ( (iGrenade = find_ent_by_class(iGrenade, "grenade")) > 0 ) {
					...
					if ( id == entity_get_edict(iGrenade, EV_ENT_owner) && equal(model, "models/w_hegrenade.mdl") ) {
						entity_set_model(iGrenade, gPenguin_w)

						// Set speed here since it gets called so much
						gNadeSpeed = get_pcvar_float(gPcvarNadeSpeed)
						...
						// Set the fuse
						set_task(get_pcvar_float(gPcvarFuse), "unpause_nade", iGrenade, parm, 2)
					}
				}
			}
			...
			if ( seconds > 0.0 ) {
				...
				set_task(seconds, "penguin_weapons", id)
			}
		}
...
public cooldown(parm[])
{
	...
	if ( seconds > 0.0 ) {
		// Proof From Hud			
		#if SEND_COOLDOWN
			PenguinUsedTime[id] = get_gametime()
		#endif		
		sh_set_cooldown(id, seconds)
	}*/
...
public sh_round_end()
{
	...
	get_players(players, player_num, "ah")
	
	for (new i = 0; i < player_num; i++) {
		id = players[i]
		
		if (gHasPenguinPower[id] && is_user_alive(id) && is_user_connected(id) )
			gPlayerInCooldown[id] = false
	}
}
