...
new gSpriteTrail
new gPcvarGrenadeTimer, gPcvarCooldown
#if SEND_COOLDOWN
	new Float:PenguinUsedTime[SH_MAXSLOTS+1]
#endif
//------------------------------------------------------------------------------------------------
...
public plugin_init()
{
	...
	register_cvar("penguin_grenademult", "2.5")
	gPcvarGrenadeTimer	= register_cvar("penguin_grenadetimer", "25.0")
	gPcvarCooldown		= register_cvar("penguin_cooldown", "25.0")
	register_cvar("penguin_fuse", "6.0")
	register_cvar("penguin_nadespeed", "950")
...
public sh_client_spawn(id)
{
	if ( is_user_alive(id) && gHasPenguinPower[id] ) {
		...
		set_task(0.3, "cooldownpenguinfalse", id)
	}
}

public cooldownpenguinfalse(id)
	gPlayerInCooldown[id] = false
...
public sendPenguinCooldown(id)
{
	...
	if (gPlayerInCooldown[id])
		cooldown = floatround( get_pcvar_num(gPcvarGrenadeTimer) - get_gametime() + PenguinUsedTime[id] + 0.4 )
	else
		cooldown = -1
	return cooldown
}
...
{
	if ( !shModActive() || !gHasPenguinPower[id] || gPlayerInCooldown[id] ) return
...
public on_AmmoX(id)
{
	if ( !shModActive() || !is_user_alive(id) || !sh_is_inround() ) return
