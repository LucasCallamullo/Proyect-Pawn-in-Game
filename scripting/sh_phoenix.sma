...
public plugin_init()
{
	...
	new pcvarLevel = register_cvar("phoenix_level", "9")
	register_cvar("phoenix_cooldown", "120")
	register_cvar("phoenix_radius", "375")
	register_cvar("phoenix_maxdamage", "120")
...
public sh_client_death(victim)
{
	...
	g_userTeam[victim] = get_user_team(victim)

	// Save users origin on death
	get_user_origin(victim, g_savedOrigin[victim])
	g_savedOrigin[victim][2] += 8

	// Look for self to raise from dead
	if ( !is_user_alive(victim) && g_hasPhoenix[victim] && !g_phoenixPowerUsed[victim] ) {
		new parm[1]
		parm[0] = victim
		...
		uchiha revenge = 1.2
		torneo = 1.5 */
		set_task(0.7, "phoenix_respawn", 0, parm, 1)
	}
}
//----------------------------------------------------------------------------------------------
public phoenix_respawn(parm[])
{
	...
	sh_chat_message(id, gHeroID, "Usaste el Poder del Ave %s para Volver de las Cenizas!", g_heroName)
...
public phoenix_teamcheck(parm[])
{
	...
	if ( g_userTeam[id] != get_user_team(id) ) {
		...
		sh_chat_message(id, gHeroID, "Cambiaste de equipo y usaste %s Respawn, ahora morirás!", g_heroName)
		user_kill(id, 1)
...
public positionChangeCheck(id)
{
	...
	if ( g_lastPosition[id][0] == origin[0] && g_lastPosition[id][1] == origin[1] && g_lastPosition[id][2] == origin[2] && is_user_alive(id) ) {
		user_kill(id, 1)
		sh_chat_message(id, gHeroID, "Te moriste por quedar bugeado en el respawn del %s.", g_heroName)
	}
}
//----------------------------------------------------------------------------------------------
public BlowUp(id)
	...
	for (new a = 1; a <= SH_MAXSLOTS; a++) {
		if ( is_user_alive(a) && a != id && (get_user_team(id) != get_user_team(a) || FFOn) ) {
			...
			if ( distanceBetween < dmgRadius ) {
				...
				show_hudmessage(a, "%s fue Re-Nacido usando el poder de Fénix!", name)
