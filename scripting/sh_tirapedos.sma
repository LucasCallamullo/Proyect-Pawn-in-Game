...
//models probando
new const gFartmanWeapon[] = "models/shmod/axesg_v.mdl"

new batmanID
new bool:gHasBatman[SH_MAXSLOTS+1]
new bool:gTiraPedosSelected[SH_MAXSLOTS+1] 

#if SEND_COOLDOWN
	new Float:FartmanUsedTime[SH_MAXSLOTS+1]
#endif
//----------------------------------------------------------------------------------------------
public plugin_init()
{
	...
	set_task(0.3, "cache_idTP");   		// we need to let superhero cache all the heros to avoid issues
}
...
public plugin_precache()
{
	...
	precache_model("models/shmod/axesg_v.mdl")
	precache_model("models/shmod/w_axesg.mdl")
}
//----------------------------------------------------------------------------------------------
	...
	if ( gHeroID == heroID ) {
		switch(mode) {
			case SH_HERO_ADD: {
				...
				fartman_weapon(id)
				switch_model(id)
				gTiraPedosSelected[id] = HasFartman[id]
...
public sh_client_spawn(id)
{
	...
	if ( shModActive() && HasFartman[id] && is_user_alive(id) ) {
		...
		//set_task(0.1, "fartman_weapon", id)
	}
}
#if SEND_COOLDOWN
public sendFartmanCooldown(id)
{
	...
	if (gPlayerInCooldown[id])
		cooldown = floatround( get_pcvar_num(CvarGrenadeReset) - get_gametime() + FartmanUsedTime[id] + 0.4 )
	else
		cooldown = -1
	return cooldown
}
...
{
	if ( !sh_is_active() || !HasFartman[id] ) return
...
switch_model(id)
{
	if (!sh_is_active() || !is_user_alive(id) || !HasFartman[id] ) return
	...
	if (get_user_weapon(id) == CSW_SMOKEGRENADE) {
		...
		set_pev(id, pev_viewmodel2, gFartmanWeapon)
	}
} 
//----------------------------------------------------------------------------------------------
...
public on_ammox(id)
{
	if ( !shModActive() || !is_user_alive(id) || !HasFartman[id] ) return
	...
	if ( read_data(1) == AMMOX_SMOKEGRENADE ) {
		...
		if ( iAmmoCount == 0 && !gBlockGiveTask[id] ) {
			...
			if ( !gPlayerInCooldown[id] ) {
				...
				while ( (iGrenade = find_ent_by_class(iGrenade, "grenade")) > 0 ) {
					...
					if ( id == entity_get_edict(iGrenade, EV_ENT_owner) && equal(model, "models/w_smokegrenade.mdl") ) {
						entity_set_model(iGrenade, "models/shmod/w_axesg.mdl")
					}
				}
			}
			...
			if ( seconds > 0.0 ) {
				// Proof From Hud			
				#if SEND_COOLDOWN
					FartmanUsedTime[id] = get_gametime()
				#endif	
...
{
	// if ( !gHasBatman[id] ) return
	 
	if ( gHasBatman[id] && gTiraPedosSelected[id] ) {
		new heronametodrop[]= "Batman"
